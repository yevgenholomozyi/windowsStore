...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var modalState = {}; // the object to keep the state of choosen options by the user for calculatingn the price
...
var changeModalState = function changeModalState(state) {
  ...
  function bindActionToElems(event, elem, prop) {
    // function to remember the state of choosen options
    elem.forEach(function (item, i) {
      item.addEventListener(event, function () {
        switch (item.nodeName) {
          case 'SPAN':
            // choose the value of founded checkbox
            state[prop] = i;
            break;

          case 'INPUT':
            // remeber the value of inputs
            if (item.getAttribute('type') === 'checkbox') {
              // choose the windowProfile, as only they have checkboxes
              i === 0 ? state[prop] = "Холодное" : state[prop] = "Теплое"; // remember the value of the windowProfile

              elem.forEach(function (box, j) {
                // clear inputs exept choosen
                box.checked = false;
...
var forms = function forms(state) {
  ...
  phoneInputs.forEach(function (item) {
    var phoneError = document.createElement('div'),
        // creating an element to show a message
    parent = item.parentNode,
        // phoneError will be added to the parent node
    parentNodes = parent.childNodes,
        // find all childNodes of the parent to transform it into the array
    ...
    item.addEventListener('input', function () {
      if (item.value.match(/\D/) && !parentElements.includes(phoneError)) {
        // only digits must be inserted
        item.style.border = "1px solid red";
        item.value = '';
    ...
    return regeneratorRuntime.async(function postData$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            // without 'async' the result of the function will be sent empty
            document.querySelector('.status').textContent = message.loading; // work with the fetch

            _context.next = 3;
            return regeneratorRuntime.awrap(fetch(url, {
              // 'await' is waiting for a response from the server
              method: 'POST',
              body: data
            }));
  ...
  form.forEach(function (item) {
    return item.addEventListener('submit', function (e) {
      ...
      var statusMessage = document.createElement('div'); // to show the status of the message
      ...
      .then(function (result) {
        return statusMessage.textContent.sucsess;
      }).catch(function (result) {
        return statusMessage.textContent.failrue;
      }).finally(function () {
        clearInputs();
        setTimeout(function () {
...
var images = function images() {
  var imgPopup = document.createElement('div'),
      // element to be created on the click
  workSection = document.querySelector('.works'),
      // section were the element will be created
  ...
  workSection.addEventListener('click', function (e) {
    ...
    if (target && target.classList.contains('preview')) {
      ...
      var path = target.parentNode.getAttribute('href'); // img with a class 'preview' contains in the link, we must get the value of this link
...
var modals = function modals() {
  ...
  function calcScroll() {
    ...
    var scrollWidth = helpDiv.offsetWidth - helpDiv.clientWidth; // total widt(with borders) minus content width+padding, hence we'he got the scroll width
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  function hideTabContent() {
    ...
    }); // tabs contains activity class which should be removed by default
  }
...
var timer = function timer() {
  var deadline = '2020-07-30';

  function getTimeRemaining(remainingTime) {
    var t = Date.parse(remainingTime) - Date.parse(new Date()),
    ...
    hours = Math.floor(t / (1000 * 60 * 60) % 24),
        // converts mil ser to hours; divides into 60 twice 
